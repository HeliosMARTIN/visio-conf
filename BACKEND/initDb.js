...
const usersToInsert = [
  {
    ...
    status: "active",
    password: "f4f263e439cf40925e6a412387a9472a6773c2580212a4fb50d224d3a817de17", // Mot de passe : 'mdp'
  },
  {
    uuid: uuidv4(),
    firstname: "Janny",
...
const initializeChannels = async (teams, users) => {
  ...
  for (const team of teams) {
    ...
    for (const channelData of additionalChannels) {
      ...
      console.log(`Canal "${channelData.name}" créé pour l'équipe "${team.name}"`)
    }
  }
...
const initializeChannelPosts = async (channels, users) => {
  ...
  for (const channel of channels) {
    ...
    const eligibleUsers = users.filter((u) => memberUserIds.includes(u._id.toString()))

    if (eligibleUsers.length === 0) continue

    // 2 posts par channel
    for (let i = 0; i < 2; i++) {
      const author = eligibleUsers[Math.floor(Math.random() * eligibleUsers.length)]
      ...
      // 1 réponse par post
      const responder = eligibleUsers[Math.floor(Math.random() * eligibleUsers.length)]
      const responseData = {
        postId: newPost._id,
...
const initializeFiles = async (users) => {
  ...
  for (const user of users) {
    ...
    for (const folder of rootFolders) {
      ...
      console.log(`Dossier ${folder.name} créé pour ${user.firstname} ${user.lastname}`)
      ...
      if (folder.name === "Documents") {
        ...
        const docFiles = [
          {
            ...
            ownerId: user.uuid,
            parentId: folder.id,
            path: `files/${user.uuid}/${uuidv4()}/rapport_annuel.pdf`,
            createdAt: new Date(),
            updatedAt: new Date(),
          },
        ]
        ...
        const imageFiles = [
          ...
          },
          {
            ...
            ownerId: user.uuid,
            parentId: folder.id,
            path: `files/${user.uuid}/${uuidv4()}/logo_universite.png`,
            createdAt: new Date(),
            updatedAt: new Date(),
          },
        ]
...
const initDb = async (mongooseInstance = null) => {
  let shouldCloseConnection = false

  if (!mongooseInstance) {
    // Si aucune instance mongoose n'est fournie, créer une nouvelle connexion
    ...
    // Utiliser l'instance mongoose fournie
  ...
  try {
    ...
    // Fermer la connexion uniquement si nous l'avons ouverte nous-mêmes
...
// Exécuter initDb directement si le script est appelé directement
