...
class WebRTC {
  ...
  constructor(controller, instanceName) {
    ...
    if (this.verbose || this.controller.verboseall)
      console.log(
        `INFO (${this.instanceName}): s'enregistre auprès du controleur`
      );

    this.controller.inscription(
      this,
      this.listeMessagesEmis,
      this.listeMessagesRecus
    );
  }

  traitementMessage = async (msg) => {
    if (this.verbose || this.controller.verboseall)
      console.log(`INFO (${this.instanceName}) - Traitement du message : `);
    ...
    if (typeof msg.new_call !== "undefined") {
      ...
      try {
        const call = await Call.findOne({
          ...
          .populate("in_call_members")
          .populate("call_creator");
        const userFrom = await User.findBySocketId(msg.id);

        if (!call) {
          if (this.verbose || this.controller.verboseall)
            console.log(
              "Creating new call for discussion: " +
                msg.new_call.discussion_uuid
            );
          const discussion = await Discussion.findPopulateMembersByDiscussionId(
            msg.new_call.discussion_uuid
          );
          ...
          const newCall = new Call({
            ...
            members_allowed_to_join: discussion.discussion_members || [
              userFrom,
            ],
      ...
      try {
        const call = await Call.findOne({
          ...
          .populate("in_call_members", "user_socket_id")
          .populate("call_creator");
        const userFrom = await User.findBySocketId(msg.id);
        const userTo = await User.findBySocketId(msg.send_offer.target);
        const pseudo_caller =
          userFrom.user_firstname + " " + userFrom.user_lastname;

        if (
          call &&
          userTo &&
          userTo.user_socket_id &&
          userTo.user_is_online &&
          userTo.user_socket_id !== msg.id
        ) {
          if (this.verbose || this.controller.verboseall)
            console.log(
              msg.id + " is sending offer to: " + msg.send_offer.target
            );

          const discussion = await Discussion.findPopulateMembersByDiscussionId(
            msg.send_offer.discussion_uuid
          );
          ...
          this.controller.envoie(this, {
            receive_offer: {
              ...
              in_call_members:
                call.in_call_members.map((member) => member.user_socket_id) ||
                [],
            },
      ...
      try {
        const call = await Call.findOne({
          ...
          .populate("in_call_members")
          .populate("members_allowed_to_join");
        const userFrom = await User.findBySocketId(msg.id);
        ...
        if (
          ...
          if (this.verbose || this.controller.verboseall)
            console.log(
              msg.id + " is sending answer to: " + msg.send_answer.target
            );
            ...
            if (member.user_socket_id) {
              this.controller.envoie(this, {
                call_connected_users: {
                  ...
                  connected_users: call.in_call_members.map(
                    (member) => member.user_socket_id
                  ),
                },
      ...
      try {
        ...
        if (
          ...
          if (this.verbose || this.controller.verboseall)
            console.log(
              msg.id +
                " is sending ice candidate to: " +
                msg.send_ice_candidate.target
            );
          this.controller.envoie(this, {
            receive_ice_candidate: {
              discussion_uuid: msg.send_ice_candidate.discussion_uuid,
      ...
      try {
        const call = await Call.findOne({
          discussion_uuid: msg.reject_offer.discussion,
          is_ended: false,
        });
        ...
        if (
          ...
          if (this.verbose || this.controller.verboseall)
            console.log(
              msg.id + " is rejecting offer from: " + msg.reject_offer.target
            );
          this.controller.envoie(this, {
            offer_rejected: {
              sender: msg.id,
      ...
      try {
        ...
        if (call) {
          ...
          if (userFrom) {
            if (this.verbose || this.controller.verboseall)
              console.log(
                userFrom.user_firstname +
                  " is hanging up from call: " +
                  msg.hang_up.discussion_uuid
              );
