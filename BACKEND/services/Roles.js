...
class RolesService{
    ...
    async traitementMessage(mesg){
            ...
            if(role == null) {
                ...
                if(r != null){
                    this.controleur.envoie(this, {
                        "created_role" : {
                            "role_id" : r._id, action : mesg.create_role_request.action
                        },
                        id : [mesg.id]
                    });	
                }			 
        ...
        if(typeof mesg.update_role_request != "undefined"){
            ...
            const usersToNotify = await User.find({ roles : mesg.update_role_request.role_id});
            const socketToNotify = usersToNotify.map(user => user.socket_id);
