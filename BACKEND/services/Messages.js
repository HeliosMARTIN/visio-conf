    ...
    if (mesg.message_send_request) {
      try {
        ...
        // Cas d'un message dans une discussion existante
          ...
          members = discussion.discussion_members.map((member) => member._id);
          socketIds = discussion.discussion_members.map(
            (member) => member.socket_id
          );
        }
    ...
    if (mesg.message_status_request) {
      try {
        ...
        const conv = await Discussion.findOneAndUpdate(
          ...
          {
            $set: {
              "discussion_messages.$[elem].message_status": "read",
            },
          },
          {
            arrayFilters: [{ "elem.message_status": { $ne: "read" } }],
            new: true,
          }
        ).populate({
          path: "discussion_members",
