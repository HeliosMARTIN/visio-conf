...
export default function AnnuairePage() {
  ...
  const handleMessageUser = useCallback(
    (user: User) => {
      ...
      console.log("📋 Requête discuss_list avec identifiant:", userIdentifier);
  ...
  const createNewDiscussion = useCallback(
    (user: User) => {
      ...
      setTimeout(() => {
        ...
        console.log("🚀 Redirection vers:", `/discussion?id=${discussionUuid}`);
  ...
  // Créer le handler avec useCallback pour éviter les re-créations
  const handler = useCallback(
    {
      ...
      traitementMessage: (msg: any) => {
        ...
        if (msg.discuss_list_response) {
          ...
          console.log("🎯 TargetUser au moment du traitement:", targetUser);

          if (!targetUser) {
            console.log("⚠️ Pas de targetUser défini, abandon du traitement");
            return;
          }
          ...
          if (msg.discuss_list_response.etat) {
            ...
            for (let i = 0; i < discussions.length; i++) {
              ...
              const currentUserInDiscussion = disc.discussion_members.some(
                (member: any) =>
              ...
              const targetUserInDiscussion = disc.discussion_members.some(
                (member: any) => {
                  const matchByEmail = member.email === targetUser.email;
                  const matchById =
                    (targetUser.id && member.id === targetUser.id) ||
                    (targetUser._id && member._id === targetUser._id);
              ...
              if (targetUserInDiscussion) {
                ...
                console.log(
                  `🎯 Discussion existante trouvée: ${disc.discussion_uuid}`
                );
                break;
              }
            }
            ...
            if (existingDiscussion) {
              console.log("🎉 Discussion existante trouvée!");
              console.log(
              ...
              console.log(
                "🆕 Aucune discussion existante trouvée, création d'une nouvelle..."
              );
            ...
            console.log("🆕 Création d'une nouvelle discussion par défaut...");
            createNewDiscussion(targetUser);
          }
