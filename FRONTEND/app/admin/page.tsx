...
export default function HomeAdmin() {
    const tabs = [
        {name : "Utilisateurs", icon : <UsersRound size={60} color="#0272DA"/>, color : "#0272DA"},
        {name : "Rôles", icon : <Drama size={60} color="#63B367"/>, color : "#63B367"},
        {name : "Permissions", icon : <ListChecks size={60} color="#DA1F63"/>, color : "#DA1F63"},
        {name : "Groupes", icon : <MessagesSquare size={60} color="#444447"/>, color : "#444447"},
    ]
    return(
            ...
            <div className={styles.tabsContainer}>
                {
                    tabs.map((tab) => {
                        return(
                            <div className={styles.tab} style={{borderColor: tab.color}}>
                                {tab.name}
                                {tab.icon}
                            </div>
                        )
