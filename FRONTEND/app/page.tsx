          ...
          fetch(msg.upload_response.signedUrl, {
            ...
            .then((response) => {
              if (response.ok) {
                ...
                setUploadMessage("S3 upload failed: " + response.statusText);
              }
            })
            .catch((error) => {
              setUploadMessage("S3 upload error: " + error.message);
            });
        } else {
          setUploadMessage("Upload failed: " + msg.upload_response.error);
        }
      }
  ...
  return (
    <div className={styles.page}>
      <main className={styles.main}>
        ...
        {uploadMessage && <div className={styles.info}>{uploadMessage}</div>}
        <button onClick={fetchUsersList}>Fetch Users List</button>
        <UsersList users={users} currentUserEmail={currentUser?.email || ""} />
