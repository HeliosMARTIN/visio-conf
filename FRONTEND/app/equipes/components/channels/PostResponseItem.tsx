...
export default function PostResponseItem({ response, currentUserId, id }: PostResponseItemProps) {
  ...
  return (
    <div id={id} className={`${styles.responseItem} ${isAuthor ? styles.authorResponse : ""}`}>
        ...
        <div className={styles.userInfo}>
          ...
          <div>
            <span className={styles.userName}>
              {response.authorName}
              {isAuthor && <span className={styles.authorBadge}>Vous</span>}
            </span>
            <span className={styles.timestamp}>{formatDate(response.createdAt)}</span>
          </div>
        </div>
      </div>
