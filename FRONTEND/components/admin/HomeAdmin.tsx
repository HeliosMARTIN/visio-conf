...
export default function HomeAdmin({ user }: { user: any }) {
    ...
    const listeMessageEmis = ["users_list_request", "user_perms_request"];
    ...
    const handler = {
        ...
        traitementMessage: (msg: {
            ...
            if (msg.update_user_roles_response || msg.updated_role) {
                ...
                // Notification pour la mise à jour des rôles
                createNotification({
                    type: "role_update",
            ...
            if (msg.users_list_response) {
                const nbOnlineUsers = msg.users_list_response.users.reduce(
                    (acc: number, user: any) => {
                ...
                // Notification pour les changements d'état des utilisateurs
                msg.users_list_response.users.forEach((updatedUser: any) => {
                    const previousUser = userList.find(
                        (u: any) => u.id === updatedUser.id
                    );
                    if (
                        previousUser &&
                        previousUser.online !== updatedUser.online
                    ) {
                        createNotification({
                            type: "system",
                            title: `${updatedUser.firstname} ${updatedUser.lastname}`,
                            message: updatedUser.online
    ...
    return isAdmin ? (
        !selectedTab ? (
            ...
            <div style={{ display: "flex" }}>
                ...
                {selectedTab === "Utilisateurs" && (
                    <HomeUserGestion userPerms={userPerms} />
                )}
                {selectedTab === "Rôles" && (
                    <HomeRoleGestion userPerms={userPerms} />
                )}
                {selectedTab === "Permissions" && (
                    <HomePermGestion userPerms={userPerms} />
                )}
                {selectedTab === "Equipes" && (
                    <HomeTeamGestion userPerms={userPerms} />
                )}
            </div>
        )
    ) : (
        <div className={styles.forbiddenAccess}>
            ...
            <Typography
                style={{ fontSize: "40px", fontWeight: 800, color: "#ff0000" }}
            >
                Vous n'êtes pas autorisé à accéder à cette page !
            </Typography>
            <p style={{ fontSize: "20px", fontWeight: 500, color: "#223A6A" }}>
                S'il s'agit d'une erreur, veuillez contacter un administrateur
                de l'application.
            </p>
        </div>
    );
