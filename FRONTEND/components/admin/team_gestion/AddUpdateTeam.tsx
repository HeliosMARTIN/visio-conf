...
export default function AddUpdateTeam({
    ...
    const handler = {
        ...
        traitementMessage: (msg: {
            ...
            if (msg.team_create_response) {
                if (msg.team_create_response.etat) {
                    createNotification({
                        type: "team_invite",
                        title: "Équipe créée",
                        message: `L'équipe ${teamName} a été créée avec succès`,
                        priority: "medium",
                    });

                    // Notifier les membres sélectionnés
                    selectedUsers.forEach((user) => {
                        createNotification({
                            type: "team_invite",
                            title: "Invitation à une équipe",
                            message: `Vous avez été ajouté à l'équipe ${teamName}`,
                            priority: "medium",
                            actionUrl: "/equipes",
            ...
            if (msg.team_update_team) {
                if (msg.team_update_team.etat) {
                    createNotification({
                        type: "team_invite",
                        title: "Équipe mise à jour",
                        message: `L'équipe ${teamName} a été mise à jour avec succès`,
                        priority: "medium",
                    });
                    ...
                    createNotification({
                        type: "system",
                        title: "Erreur",
                        message: "Impossible de mettre à jour l'équipe",
                        priority: "high",
                    });
                }
                setAddUpdateTeam(false);
    ...
    const handleAddUpdateRole = () => {
        if (teamName) {
            ...
            setAlertMessage(
                "Vous ne pouvez créer ou modifier une équipe sans mettre un nom !"
            );
    ...
    return (
        <div className={styles.container}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                >
                    <Check size={26} color="white" /> Valider
                </button>
            </div>
            <CustomSnackBar
                open={openAlert}
